...
3. **React dashboard** (`src/`) — a Vite + TypeScript + Tailwind SPA. All dashboard components read data through `useAnalysisData()` (`src/contexts/AnalysisDataContext.tsx`), which fetches `/live-dataset.json` at runtime and falls back to the static snapshot in `src/data/analysisData.ts` if that file doesn't exist yet or fails to load (e.g. before the first cron run, or `GITHUB_TOKEN` isn't configured in Vercel). **Never import from `@/data/analysisData` directly in a component** — always go through the context, or the component will silently ignore live data.
...
3. Deep-enriches the top 25 (by stars) with contributor count, commit windows (30/90/365d), and community-profile health files (readme/license/contributing/code-of-conduct/issue-template/security) — this is 5 extra API calls per repo, so it's capped at 25/language (300 total) to stay within GitHub's rate limit and the function's execution time. `activityScore`/`healthScore`/`overallScore`/`growthSignal` only exist for this enriched subset; language-level averages for those fields are means over the 25-sample, not the full 100.
...
Without `GITHUB_TOKEN` set, `/api/refresh-repos` returns 500 and `public/live-dataset.json` never gets created — the dashboard just keeps showing the static snapshot with a "Snapshot data" badge instead of "Live", which is expected, not a bug.

**Known trade-off**: persisting the dataset as a committed file (rather than a database) means every refresh is a real git commit to `main`, which triggers a full Vercel rebuild. This was a deliberate choice to avoid requiring the user to provision a KV/Blob store; revisit if commit-noise or rebuild frequency becomes a problem.
...
**Dashboard shell**: `src/pages/Dashboard.tsx` wraps everything in `AnalysisDataProvider` (the live-data context), then renders `DashboardContent`, which holds `activeSection` state and swaps in one of the section components (`Overview`, `LanguageExplorer`, `CorrelationAnalysis`, `EnterpriseReadiness`, `RepositoryExplorer`, `Visualizations3D`, `HowToUse`) from `src/components/dashboard/`. There is no routing per section; adding a new dashboard tab means adding an entry to the `navigation` array and a `case` in `renderSection()`.

**Data flow**: `useAnalysisData()` (`src/contexts/AnalysisDataContext.tsx`) is the single source of truth for `languageData`, `topRepositories`, `correlationData`, `segmentData`, `healthIndicators` — every dashboard component gets these from the hook, never from a static import. Under the hood it fetches `/live-dataset.json` (shape: `src/types/liveDataset.ts`) and runs it through `src/lib/transformLiveDataset.ts` to match the exact shapes `src/data/analysisData.ts` used to export directly; on fetch failure it falls back to those static exports unchanged. `src/data/analysisData.ts` itself is now only the fallback/seed data source, not something components read directly.
